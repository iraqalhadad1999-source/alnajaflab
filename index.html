...
عِراق مُحَمد عيسى, [11/9/2025 9:10 PM]
        ...
        <div class="main-content">
            ...
            <div class="page active" id="dashboardPage">
                ...
                <div class="stats-cards">
                    ...
                    <div class="stat-card">
                        <h3>المواد قيد التحليل</h3>
                        <div class="value" id="analyzingMaterials">0</div>
                    </div>
                    <div class="stat-card">
                ...
                <div class="reports-content">
                    <div class="form-group">
                        ...
                        <select id="reportType">
                            ...
                            <option value="analyzing">تقرير المواد قيد التحليل</option>
...
<option value="labs">تقرير المختبرات</option>
            ...
            <div class="page" id="chemistryPage">
                ...
                <div id="selectedMaterialInfo" class="material-info" style="display: none;">
                    ...
                    <p><strong>الحالة الحالية:</strong> <span id="infoCurrentStatus"></span></p>
                </div>
...
case 'accepted': return 'مقبولة';
        ...
        function loadUnprocessedMaterials() {
            ...
            const unprocessedMaterials = materials.filter(material => 
                ...
                (!material.analysisMethod  material.status === 'pending'  material.status === 'analyzing')
            );
            
            unprocessedMaterials.forEach(material => {
                const option = document.createElement('option');
                option.value = material.id;
                option.textContent = ${material.name} - ${material.supplierNumber || 'بدون رقم وارد'} (${getStatusName(material.status)});
        ...
        selectedMaterial.addEventListener('change', function() {
            ...
            if (materialId) {
                ...
                if (material) {
                    ...
                    infoCurrentStatus.textContent = getStatusName(material.status);
                    selectedMaterialInfo.style.display = 'block';
                    
                    // تعيين الحالة الحالية في القائمة المنسدلة
                    document.getElementById('materialStatus').value = material.status || 'analyzing';
                }
            } else {
...
document.getElementById(`lab${i}Doctors`).textContent = labDoctors.length;
        ...
        function loadDashboardData() {
            ...
            document.getElementById('analyzingMaterials').textContent = materials.filter(m => m.status === 'analyzing').length;
...
function loadLabMaterials() {
        ...
        warehouseForm.addEventListener('submit', function(e) {
            ...
            setLocalStorage('activities', JSON.stringify(activities));
...
preventDefault();
        ...
        document.getElementById('chemistryForm').addEventListener('submit', function(e) {
            ...
            // تسجيل النشاط
            activities.push({
                action: فحص مادة: ${materials[materialIndex].name} - ${materials[materialIndex].analysisMethod} (${getStatusName(materials[materialIndex].status)}),
                user: currentUser.name,
                date: new Date().toLocaleDateString('ar-EG')
            });
            setLocalStorage('activities', activities);
